...
class TestAliasResolution:
    def test_store_name_maps_to_company(self):
        samples = _make_samples([{"store_name": "ACME Corp", "date": "01/01/2024", "address": "123 St", "total": "10.00"}])
    ...
    def test_merchant_maps_to_company(self):
        samples = _make_samples([{"merchant": "My Shop", "date": "01/01/2024", "address": "Addr", "total": "5.00"}])
        result = DatasetNormalizer(coverage_threshold=0.0).normalise(samples)
        assert result[0][1]["company"] == "My Shop"

    def test_grand_total_maps_to_total(self):
        samples = _make_samples([{"company": "Shop", "date": "01/01/2024", "address": "Addr", "grand_total": "99.99"}])
        result = DatasetNormalizer(coverage_threshold=0.0).normalise(samples)
        assert result[0][1]["total"] == "99.99"

    def test_receipt_date_maps_to_date(self):
        samples = _make_samples([{"company": "Shop", "receipt_date": "12/12/2023", "address": "Addr", "total": "1.00"}])
        result = DatasetNormalizer(coverage_threshold=0.0).normalise(samples)
...
class TestNAHandling:
    def test_na_becomes_empty_string(self):
        samples = _make_samples([{"company": "N/A", "date": "01/01/2024", "address": "Addr", "total": "1.00"}])
        result = DatasetNormalizer(coverage_threshold=0.0).normalise(samples)
    ...
    def test_null_becomes_empty_string(self):
        samples = _make_samples([{"company": "null", "date": "01/01/2024", "address": "Addr", "total": "1.00"}])
        result = DatasetNormalizer(coverage_threshold=0.0).normalise(samples)
        assert result[0][1]["company"] == ""

    def test_none_value_becomes_empty_string(self):
        samples = _make_samples([{"company": None, "date": "01/01/2024", "address": "Addr", "total": "1.00"}])
        result = DatasetNormalizer(coverage_threshold=0.0).normalise(samples)
        assert result[0][1]["company"] == ""
...
class TestCurrencyStripping:
    def _sample(self, total: str) -> list[tuple[Path, dict]]:
        return _make_samples([{"company": "Shop", "date": "01/01/2024", "address": "Addr", "total": total}])

    def test_strip_rm_prefix(self):
        result = DatasetNormalizer(coverage_threshold=0.0, strip_currency=True).normalise(self._sample("RM 47.80"))
        assert result[0][1]["total"] == "47.80"

    def test_strip_dollar_prefix(self):
        result = DatasetNormalizer(coverage_threshold=0.0, strip_currency=True).normalise(self._sample("$10.50"))
        assert result[0][1]["total"] == "10.50"

    def test_strip_s_dollar_prefix(self):
        result = DatasetNormalizer(coverage_threshold=0.0, strip_currency=True).normalise(self._sample("S$25.00"))
        assert result[0][1]["total"] == "25.00"

    def test_no_strip_when_disabled(self):
        result = DatasetNormalizer(coverage_threshold=0.0, strip_currency=False).normalise(self._sample("RM 47.80"))
        assert result[0][1]["total"] == "RM 47.80"

    def test_no_strip_when_disabled_dollar(self):
        result = DatasetNormalizer(coverage_threshold=0.0, strip_currency=False).normalise(self._sample("$10.50"))
        assert result[0][1]["total"] == "$10.50"


class TestConvenienceWrapper:
    def test_normalise_samples_returns_same_as_class(self):
        samples = _make_samples([{"store_name": "X", "date": "01/01/2024", "address": "A", "total": "1.00"}])
