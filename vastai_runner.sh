...
if [[ -f "$ENV_FILE" ]]; then
    ...
    echo "[env] No .env.cloud found at $ENV_FILE — relying on exported env vars"
fi
...
$VASTAI_CMD search offers \
    ...
    || { log "  vastai stderr: $(cat "$_SEARCH_ERR")"; rm -f "$_SEARCH_TMP" "$_SEARCH_ERR"; die "vastai search offers failed."; }
...
# Block 1: shebang + set (no expansion needed)
